...
func defaultDecryptorOptions() *decryptorOptions {
	return &decryptorOptions{
		hash:       hash.Hash(strings.ToLower(conf.Get(defaultDecryptorHashKey))),
		padding:    EncryptPadding(strings.ToUpper(conf.Get(defaultDecryptorPaddingKey))),
		label:      []byte(conf.Get(defaultDecryptorLabelKey)),
